...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        for hit in hits:
            ...
            #print(hit["fields"]["_ltrlog"][0])
            #print(hit["fields"]["_ltrlog"][0]["log_entry"])
           
            if "value" in hit["fields"]["_ltrlog"][0]["log_entry"][0]:
                 name_value = hit["fields"]["_ltrlog"][0]["log_entry"][0]["value"]
                 sales_rank = hit["fields"]["_ltrlog"][0]["log_entry"][1]["value"]
                 #print(name_value, sales_rank)
                 for doc_id in query_doc_ids:
                    feature_results["doc_id"].append(doc_id)  # capture the doc id so we can join later
                    feature_results["query_id"].append(query_id)
                    feature_results["sku"].append(doc_id)  
                    feature_results["name_match"].append(name_value)
                    feature_results["salesRankShortTerm"].append(sales_rank)

        frame = pd.DataFrame(feature_results)
        return frame.astype({'doc_id': 'int64', 'query_id': 'int64', 'sku': 'int64', 'name_match' : 'float', 'salesRankShortTerm' : 'float'})
        # IMPLEMENT_END
